...
# 行動の価値関数の獲得
    ...
    def update_model(self, num, step, done):
        ...
        if num!=0 and self.replay_flag == 1:
            ...
            x = np.array(x)  # タプルをNumPy配列に変換
            x = tensor(x.reshape((self.batch_num, -1)).astype(np.float32), requires_grad=True).to(device)
            new_x = np.array(new_x)
            new_x = tensor(new_x.reshape((self.batch_num, -1)).astype(np.float32)).to(device)
            # 1, メインネットワークからQ値を取得
            ...
            indices_cpu = indices_next.cpu().numpy()
            ...
            for i in range(self.batch_num):
                ...
                targets[i, int(a):int(a+1)] = r + self.gamma * next_q_values[i, int(indices_cpu[i]):int(indices_cpu[i]+1)] * (1 - bool_dones)
            # 先ほど計算したtarget値を変換
            target = tensor(np.array(targets).reshape((self.batch_num, -1)).astype(np.float32)).to(device)
            # 勾配をクリアにする
            self.optimizer.zero_grad()
            # lossの計算をする
            loss = criterion(model_array, target)
            weights_tensor = torch.tensor(weights, dtype=torch.float32)
            weights_tensor = weights_tensor.unsqueeze(1)
            loss = torch.sum(weights_tensor * loss.cpu())
            # TD誤差に重みづけ
            loss.backward()
            self.optimizer.step()
            ...
            errors = (target - model_array).abs().detach().cpu().numpy()
            # 優先度の更新
